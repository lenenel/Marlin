...
void MarlinUI::tft_idle() {
    ...
    if (draw_menu_navigation) {
      add_control(64, TFT_HEIGHT - 40, PAGE_UP, imgPageUp, encoderTopLine > 0);
      add_control(224, TFT_HEIGHT - 40, PAGE_DOWN, imgPageDown, encoderTopLine + LCD_HEIGHT < screen_items);
      add_control(144, TFT_HEIGHT - 40, BACK, imgBack);
...
void MarlinUI::draw_status_screen() {
  ...
  if (TERN0(LCD_SHOW_E_TOTAL, printingIsActive())) {
    ...
    tft.add_text(106 - tft_string.width(), 3, nhx ? COLOR_AXIS_NOT_HOMED : COLOR_AXIS_HOMED, tft_string);

    tft.add_text(114, 3, COLOR_AXIS_HOMED , "Y");
    const bool nhy = axis_should_home(Y_AXIS);
    tft_string.set(blink && nhy ? "?" : ftostr4sign(LOGICAL_Y_POSITION(current_position.y)));
    tft.add_text(206 - tft_string.width(), 3, nhy ? COLOR_AXIS_NOT_HOMED : COLOR_AXIS_HOMED, tft_string);
  }
  tft.add_text(214, 3, COLOR_AXIS_HOMED , "Z");
  uint16_t offset = 32;
  ...
  tft.add_text(316 - tft_string.width(), 3, nhz ? COLOR_AXIS_NOT_HOMED : COLOR_AXIS_HOMED, tft_string);
  TERN_(TOUCH_SCREEN, touch.add_control(MOVE_AXIS, 4, y, TFT_WIDTH - 8, FONT_LINE_HEIGHT));

  y += 48;
  // feed rate
  tft.canvas(60, y, 100, 32);
  tft.set_background(COLOR_BACKGROUND);
  ...
  TERN_(TOUCH_SCREEN, touch.add_control(FEEDRATE, 60, 176, 100, 32));

  // flow rate
  tft.canvas(160, y, 100, 32);
  tft.set_background(COLOR_BACKGROUND);
  ...
  tft_string.add('%');
  tft.add_text(36, 1, color , tft_string);
  TERN_(TOUCH_SCREEN, touch.add_control(FLOWRATE, 160, 176, 100, 32, active_extruder));

  y += 44;

  #if ENABLED(TOUCH_SCREEN)
    add_control(244, y, menu_main, imgSettings);
...
void MenuEditItemBase::draw_edit_screen(FSTR_P const fstr, const char * const value/*=nullptr*/) {
  ...
  if (ui.can_show_slider()) {
    ...
    #define SLIDER_LENGTH 176
    #define SLIDER_Y_POSITION 286
...
void MarlinUI::move_axis_screen() {
  ...
  spacing = (TFT_WIDTH - X_MARGIN * 2 - 4 * BTN_WIDTH) / 3;
  x += BTN_WIDTH + spacing;
  ...
  // ROW 2 -> "Y+" CurY
  y += BTN_HEIGHT + (TFT_HEIGHT - Y_MARGIN * 2 - 6 * BTN_HEIGHT) / 5;
  x = (TFT_WIDTH - X_MARGIN * 2 - BTN_WIDTH) / 2;
  ...
  x = BTN_WIDTH + 32;
  // aligned with x+
  drawBtn(x, TFT_HEIGHT - Y_MARGIN - BTN_HEIGHT, "off", (intptr_t)disable_steppers, imgCancel, COLOR_WHITE, !busy);
